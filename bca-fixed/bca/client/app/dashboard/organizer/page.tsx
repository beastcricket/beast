...
export default function OrganizerDashboard() {
  ...
  const fetchPlayers = async () => {
    if (!sel) return;
    try { const r = await api.get(`/auctions/${sel._id}/players`); setPlayers(r.data.players || []); } catch {}
  };
  const fetchTeams = async () => {
    if (!sel) return;
    try { const r = await api.get(`/auctions/${sel._id}/teams`); setTeams(r.data.teams || []); } catch {}
  };
  ...
  const saveAuction = async (e: React.FormEvent) => {
    ...
    try {
      if (editAuction) {
        ...
        toast.success('Updated!'); setEditAuction(null);
      } else {
        const r = await api.post('/auctions', aForm);
        setAuctions(p => [r.data.auction, ...p]);
        setSel(r.data.auction); toast.success('Auction created! 🏏');
      }
      setTab('players');
  ...
  const addPlayer = async (e: React.FormEvent) => {
    e.preventDefault(); if (!sel) return; setLoading(true);
    try {
      ...
      setPlayers(p => [...p, r.data.player]); toast.success('Player added!');
                      ...
                      <div className="bg-glass-premium rounded-xl p-6 gold-edge border-gold-subtle">
                        ...
                        <form onSubmit={saveTeam}>
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                            ...
                            {!editTeam && <div><label className={LBL}>Logo</label><input type="file" accept="image/*" onChange={e=>setTLogo(e.target.files?.[0]||null)} className="w-full text-muted-foreground text-xs file:mr-2 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-primary/10 file:text-primary file:font-heading file:text-xs cursor-pointer"/></div>}
                          </div>
                        ...
                        <div className="p-5">
                          <div className="flex items-center gap-3 mb-4">
                            {team.logo
                              ...
                              : <div className="w-12 h-12 rounded-xl flex items-center justify-center text-black font-bold font-heading flex-shrink-0" style={{ background:`linear-gradient(135deg,${team.primaryColor},${team.primaryColor}88)`, fontSize:18 }}>{team.shortName?.slice(0,2)}</div>}
